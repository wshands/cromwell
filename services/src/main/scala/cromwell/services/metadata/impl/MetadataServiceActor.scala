...
final case class MetadataServiceActor(serviceConfig: Config, globalConfig: Config, serviceRegistryActor: ActorRef)
  ...
  private val metadataSummaryRefreshInterval: Option[FiniteDuration] = {
    val duration = serviceConfig.getOrElse("metadata-summary-refresh-interval", 2.seconds)
  ...
  def receive = {
    ...
    case RefreshSummary => summaryActor foreach {
      _ ! SummarizeMetadata(metadataSummaryRefreshLimit, sender())
    }
